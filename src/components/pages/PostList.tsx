...
const PostList = () => {
  ...
  return (
    ...
    <div className="flex justify-between items-center mb-4">
      <h2 className="text-2xl font-bold">📋 {catename} </h2>
    ...
    <ul className="divide-y divide-base-300">
      {posts.map(post => (
        <li key={post.id} className="py-3">
          <a href={`/posts/${post.id}`} className="text-lg font-semibold hover:text-blue-400">
            {post.title}
          </a>
          <p className="text-sm text-gray-400">조회수 {post.views}</p>
        </li>
      ))}
    </ul>
  </div>
