    ...
    return (
      <section className="min-h-screen bg-base-200 text-base-content px-8 py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          {skills.map((skill) => (
            <div key={skill.name} className="bg-white dark:bg-zinc-800 p-4 rounded-xl shadow-md">
                <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-1">{skill.name}</h3>
                <StarRating level={skill.level} />
                <p className="whitespace-pre-line text-sm text-gray-600 dark:text-gray-300 mt-2">{skill.description}</p>
            </div>
            ))}
        </div>
      </section>
